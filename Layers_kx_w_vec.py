...
#class Layers_kx_w_vec(Wavefield_kx_w.Wavefield_kx_w):
class Layers_kx_w_vec(Wavefield_kx_w):  
    """
    Layers
    ...
    def __init__(self,nt,dt,nr,dx,dzvec,cpvec,csvec,rovec):
        #Wavefield_kx_w.Wavefield_kx_w.__init__(self,nt,dt,nr,dx)
    ...
    def Plot_model(self,savetikz=None):
        """
        Plot 1D Model.
        """
        ...
        if savetikz != None:
            tikz_save(savetikz+str(m)+'.tex',
                      figureheight = '\\figureheight',
                      figurewidth = '\\figurewidth')
        
        return Q
        
    def L1P_kx_w(self,cp,cs,ro,Kx,Om,nf,nk):
        """
        L1P = L1P_kx_w(self,cp,cs,ro,kx,om):
            
        Decomposition matrix L-one-plus for power-flux decomposition for all frequencies and wavenumbers.
        """
        kp  = Om/cp
        ks  = Om/cs
        kzp = (kp**2 - Kx**2)**(1/4)
        ...
        fac = np.dstack((fac,fac,fac,fac))
        L1P = np.zeros((nf,nk,4),dtype=complex)
        
        L1P[:,:,0] = 2*Kx*kzp
        L1P[:,:,1] = -(ks**2 - 2*Kx**2)/kzs
        L1P[:,:,2] = (ks**2 - 2*Kx**2)/kzp
        L1P[:,:,3] = 2*Kx*kzs
        L1P      = fac*L1P
        return L1P
    
    def L1M_kx_w(self,cp,cs,ro,Kx,Om,nf,nk):
        """
        ...
        fac = np.sqrt(ro*Om/2)/ks**2
        fac = np.dstack((fac,fac,fac,fac))
        L1M = np.zeros((nf,nk,4),dtype=complex)
        
        L1M[:,:,0] = -2*Kx*kzp
        L1M[:,:,1] = -(ks**2 - 2*Kx**2)/kzs
        L1M[:,:,2] = (ks**2 - 2*Kx**2)/kzp
        L1M[:,:,3] = -2*Kx*kzs
        L1M      = fac*L1M
        return L1M
    
    def L2P_kx_w(self,cp,cs,ro,Kx,Om,nf,nk):
        """
        L2P = L2P_kx_w(self,cp,cs,ro,kx,om):
            
        Decomposition matrix L-two-plus for power-flux decomposition for all frequencies and wavenumbers.
        """
        kp  = Om/cp
        ks  = Om/cs
        kzp = (kp**2 - Kx**2)**(1/4)
        ...
        fac = np.dstack((fac,fac,fac,fac))
        L2P = np.zeros((nf,nk,4),dtype=complex)
    ...
    def L2M_kx_w(self,cp,cs,ro,Kx,Om,nf,nk):
        """
        L2M = L2M_kx_w(self,cp,cs,ro,kx,om):
            
        Decomposition matrix L-two-minus for power-flux decomposition for all frequencies and wavenumbers.
        """
        kp  = Om/cp
        ks  = Om/cs
        kzp = (kp**2 - Kx**2)**(1/4)
        ...
        fac = 1/np.sqrt(2*ro*Om)
        fac = np.dstack((fac,fac,fac,fac))
        L2M = np.zeros((nf,nk,4),dtype=complex)
    ...
    def RT_kx_w(self,cp1,cs1,ro1,cp2,cs2,ro2,Kx,Om,conv,nf,nk):
        """
        Rplus,Tplus,Rmin,Tmin,Tplusinv = RT_kx_w(self,cp1,cs1,ro1,cp2,cs2,ro2,kx,om,conv,nf,nk)
        
        Compute scattering matrices from above and below for a horizontal interface, and all wavenumbers and frequencies.
        ...
        if cp1==cp2 and cs1==cs2 and ro1==ro2:
            Rplus = np.zeros((nf,nk,4))
            Rmin = np.zeros((nf,nk,4))
            # The transmission matrices are an identity matrix for all w-kx
            Tplus = np.zeros((nf,nk,4))
            Tplus[:,:,0] = 1
            Tplus[:,:,3] = 1
            Tplusinv = Tplus.copy()
            Tmin = Tplus.copy()
        ...
        # Compute L1 matrices
        #L1Pu = self.L1P_kx_w(cp1,cs1,ro1,Kx,Om,nf,nk)
        L1Pl = self.L1P_kx_w(cp2,cs2,ro2,Kx,Om,nf,nk)
        L1Mu = self.L1M_kx_w(cp1,cs1,ro1,Kx,Om,nf,nk)
        #L1Ml = self.L1M_kx_w(cp2,cs2,ro2,Kx,Om,nf,nk)
        
        # Compute L2 matrices
        #L2Pu = self.L2P_kx_w(cp1,cs1,ro1,Kx,Om,nf,nk)
        L2Pl = self.L2P_kx_w(cp2,cs2,ro2,Kx,Om,nf,nk)
        L2Mu = self.L2M_kx_w(cp1,cs1,ro1,Kx,Om,nf,nk)
        #L2Ml = self.L2M_kx_w(cp2,cs2,ro2,Kx,Om,nf,nk)
        
        # Compute N1 matrices
        # N1Pu(+kx) = - L2Mu(-kx).T = - { -J L2Mu(+kx) J }.T = { J L2Mu(+kx) J }.T
        N1Pu = L2Mu.copy()
        N1Pu[:,:,1] = -L2Mu[:,:,2]   # Transpose and minus
        N1Pu[:,:,2] = -L2Mu[:,:,1]   # Transpose and minus
        
        # N1Pl(+kx) = - L2Ml(-kx).T
        N1Pl = - self.L2M_kx_w(cp2,cs2,ro2,-Kx,Om,nf,nk)
        tmp = N1Pl.copy()           # Transpose
        N1Pl[:,:,1] = tmp[:,:,2]    # Transpose
        N1Pl[:,:,2] = tmp[:,:,1]    # Transpose
        
        # N1Mu(+kx) = L2Pu(-kx).T
        N1Mu =   self.L2P_kx_w(cp1,cs1,ro1,-Kx,Om,nf,nk)
        tmp = N1Mu.copy()           # Transpose
        N1Mu[:,:,1] = tmp[:,:,2]    # Transpose
        N1Mu[:,:,2] = tmp[:,:,1]    # Transpose
        ...
        N2Pu = -L1Mu.copy()
        N2Pu[:,:,1] =  L1Mu[:,:,2]     # Transpose and minus
        N2Pu[:,:,2] =  L1Mu[:,:,1]     # Transpose and minus
        
        # N2Pl(+kx) = L1Ml(-kx).T
        N2Pl =   self.L1M_kx_w(cp2,cs2,ro2,-Kx,Om,nf,nk)
        tmp = N2Pl.copy()           # Transpose
        N2Pl[:,:,1] = tmp[:,:,2]    # Transpose
        N2Pl[:,:,2] = tmp[:,:,1]    # Transpose
        
        # N2Mu(+kx) = -L1Pu(-kx).T
        N2Mu = - self.L1P_kx_w(cp1,cs1,ro1,-Kx,Om,nf,nk)
        tmp = N2Mu.copy()           # Transpose
        N2Mu[:,:,1] = tmp[:,:,2]    # Transpose
        N2Mu[:,:,2] = tmp[:,:,1]    # Transpose
        ...
        # Tmin = J Tplus.T J
        Tmin = Tplus.copy()
        Tmin[:,:,1] = - Tplus[:,:,2]    # Tranpose and minus
        Tmin[:,:,2] = - Tplus[:,:,1]    # Tranpose and minus
    ...
    def Layercel_kx_w(self,mul,conv,eps=None,sort=1):
        """
        ...
        print('Modelling reflection / transmission response ...')
        ...
        for n in range(0,N-1):
            ...
            rP,tP,rM,tM = self.RT_kx_w(cp1,cs1,ro1,cp2,cs2,ro2,Kxpos,Wpos,conv,nf,nk)[0:4]
        ...
        RM[nf-1,:] = RM[nf-1,:].real
        TM[nf-1,:] = TM[nf-1,:].real
        RM[:,nk-1] = RM[:,nk-1].real
        TM[:,nk-1] = TM[:,nk-1].real
...
#        TP[abs(TP)>10] = 0
        ...
        if sort == 1:
            RPfull,TPfull,RMfull,TMfull = self.Sort_kx_w(RP,TP,RM,TM)
            return RPfull,TPfull,RP,TP,RMfull,TMfull,RM,TM
    ...
    def Reverse_Kx(self,array):
        """
        Reverse the Kx-axis of an array.
        
        Inputs:
            array:  An array of the shape (nt x nr x 4) or (nt x nr).
            
        Outputs:
            array:  Input array with reversed Kx axis.
    ...
    def Gz2surf(self,z,mul=1,conv=1,eps=None):
        """
        G with at surface and receiver just below z 
        Needs testing
        """
        
        print('Modelling Greens functions to upper boundary ...')
        ...
        RP  = np.zeros((nf,nk,4),dtype=complex)
        RPP = np.zeros((nf,nk,4),dtype=complex)
        RM  = np.zeros((nf,nk,4),dtype=complex)
        ...
        for n in range(0,N):
            ...
            kzp =	np.sqrt(Wpos**2/cp1**2-Kxpos**2); 
            kzs =	np.sqrt(Wpos**2/cs1**2-Kxpos**2);
        ...
        # The highest negative frequency and highest negative wavenumber components are real-valued
        GPP[nf-1,:] = GPP[nf-1,:].real
        GMP[nf-1,:] = GMP[nf-1,:].real
        GPP[:,nk-1] = GPP[:,nk-1].real
        GMP[:,nk-1] = GMP[:,nk-1].real
        
        # Conjugate wavefields
        GPP = GPP.conj()
        GMP = GMP.conj()
        
        GMP,GPP = self.Sort_kx_w(GMP,GPP)
        ...
        GMM[:,1:,:] = GMM[:,-1:0:-1,:]    # Reverese kx

        # GMP2(kx) = GMP(-kx).T
        GMP2 = self.My_T(GMP)               # Transpose GMP
        GMP2[:,1:,:] = GMP2[:,-1:0:-1,:]    # Reverese kx
    ...
    def Gz2bot(self,z,mul=1,conv=1,eps=None):
        """
        G with at bottom and receiver just below z 
        Needs testing: Done, works fine.
        """
        
        print('Modelling Greens functions to lower boundary ...')
        ...
        cp = self.cpvec[NN-1]
        cs = self.csvec[NN-1]
        kzp =	np.sqrt(Wpos**2/cp**2-Kxpos**2)
        kzs =	np.sqrt(Wpos**2/cs**2-Kxpos**2)
            
        W[:,:,0] = np.exp(1j*kzp*dz)
        if cs != 0:
            W[:,:,3] = np.exp(1j*kzs*dz)   # for elastic layer
        else: 
            W[:,:,3] = 0                    # for purely acoustic layer
        
        # Propagate respnses from below through the bottom layer
        TM = self.My_dot(TM,W)
        RM = self.Mul_My_dot(W,RM,W)
        
        RMM  = np.zeros((nf,nk,4),dtype=complex)
        ...
        # The highest negative frequency and highest negative wavenumber components are real-valued
        GMM[nf-1,:] = GMM[nf-1,:].real
        GPM[nf-1,:] = GPM[nf-1,:].real
        GMM[:,nk-1] = GMM[:,nk-1].real
        GPM[:,nk-1] = GPM[:,nk-1].real
        ...
        GPM,GMM = self.Sort_kx_w(GPM,GMM)
        ...
        GPP[:,1:,:] = GPP[:,-1:0:-1,:]    # Reverese kx

        # GPM2(kx) = GPM(-kx).T
        GPM2 = self.My_T(GPM)               # Transpose GMP
        GPM2[:,1:,:] = GPM2[:,-1:0:-1,:]    # Reverese kx
        
        return GPM,GMM,GPM2,GPP
    
    
    # Green's function between depth z and surface
    def Gz2bound(self,z,mul=1,conv=1,eps=None,initials=[]):
        """
        ...
        print('Modelling Greens functions to upper and lower boundary ...')
        ...
        RPb  = np.zeros((nf,nk,4),dtype=complex)
        RMb  = np.zeros((nf,nk,4),dtype=complex)
        TPb = I.copy()
        TMb = I.copy()
...
#            self.Remove_layer(z+1)
        ...
        # The highest negative frequency and highest negative wavenumber components are real-valued
        GsPP[nf-1,:] = GsPP[nf-1,:].real
        GsMP[nf-1,:] = GsMP[nf-1,:].real
        GsPP[:,nk-1] = GsPP[:,nk-1].real
        GsMP[:,nk-1] = GsMP[:,nk-1].real
        
        # Conjugate wavefields
        GsPP = GsPP.conj()
        GsMP = GsMP.conj()
        
        GsMP,GsPP = self.Sort_kx_w(GsMP,GsPP)
        ...
        GsMM[:,1:,:] = GsMM[:,-1:0:-1,:]    # Reverese kx

        # GMP2(kx) = GMP(-kx).T
        GsMP2 = self.My_T(GsMP)               # Transpose GMP
        GsMP2[:,1:,:] = GsMP2[:,-1:0:-1,:]    # Reverese kx
        ...
        # The highest negative frequency and highest negative wavenumber components are real-valued
        GbMM[nf-1,:] = GbMM[nf-1,:].real
        GbPM[nf-1,:] = GbPM[nf-1,:].real
        GbMM[:,nk-1] = GbMM[:,nk-1].real
        GbPM[:,nk-1] = GbPM[:,nk-1].real
        
        # Conjugate wavefields
        GbMM = GbMM.conj()
        GbPM = GbPM.conj()
        
        GbPM,GbMM = self.Sort_kx_w(GbPM,GbMM)
        ...
        GbPP[:,1:,:] = GbPP[:,-1:0:-1,:]    # Reverese kx

        # GMP2(kx) = GMP(-kx).T
        GbPM2 = self.My_T(GbPM)               # Transpose GMP
        GbPM2[:,1:,:] = GbPM2[:,-1:0:-1,:]    # Reverese kx
    ...
    def GzRzS(self,zR,zS,mul=1,conv=1,eps=None,initials=[]):
        """
        ...
        print('Modelling Greens functions between zS and zR ...')
        ...
        # Initial fields
        RP1 = np.zeros((nf,nk,4),dtype=complex)
        RM1 = np.zeros((nf,nk,4),dtype=complex)
        # Here every frequency and every wavenumber component have an amplitude 
        # equal to one. Hence, the total wavefield has a strength of sqrt(nt*nr)
        # When an inverse fft (ifft2) is applied the wavefield is scaled by 
        # 1/sqrt(nt*nr) hence in the time domain the wavefield has the an
        ...
        RP2  = np.zeros((nf,nk,4),dtype=complex)
        RM2  = np.zeros((nf,nk,4),dtype=complex)
        TP2 = I.copy()
        TM2 = I.copy()
        ...
        RP3  = np.zeros((nf,nk,4),dtype=complex)
        RM3  = np.zeros((nf,nk,4),dtype=complex)
        TP3 = I.copy()
        TM3 = I.copy()
        ...
        def PropScat(RP,RM,TP,TM,W,I,Nstart,Nend,Wpos,Kxpos,mul,conv,nf,nk):
            """
            ...
            Input:
                ...
                Kxpos:  Meshgrid of positive wavenumbers (and frequencies).
                mul:    Set mul=1 to include internal multiples.
                conv:   Set conv=1 to include mode conversions.
                nf:     Number of positive temporal samples.
                nk:     Number of positive spatial samples.
        ...
        if zS < zR:
                ...
                if (zSold == zS):
                    if zSold < zRold:
                        ...
                        if zRold < zR:
                            ...
                            RP2,RM2,TP2,TM2 = PropScat(RP2,RM2,TP2,TM2,W,I,NRold+1,NR+1,Wpos,Kxpos,mul,conv,nf,nk)
                            
                            
                            # (3) Medium below the receiver depth level: Compute response
                            RP3,RM3,TP3,TM3 = PropScat(RP3,RM3,TP3,TM3,W,I,NR+1,NN-1,Wpos,Kxpos,mul,conv,nf,nk) 
            ...
            if check == 1:
                ...
                RP1,RM1,TP1,TM1 = PropScat(RP1,RM1,TP1,TM1,W,I,Nstart,NS+1,Wpos,Kxpos,mul,conv,nf,nk)
                
                # (2) Medium between the source and receiver depth levels
                    
                # Compute medium responses
                RP2,RM2,TP2,TM2 = PropScat(RP2,RM2,TP2,TM2,W,I,NS+1,NR+1,Wpos,Kxpos,mul,conv,nf,nk)    
            ...
            if check == 1:
                ...
                # Compute medium responses
                RP2,RM2,TP2,TM2 = PropScat(RP2,RM2,TP2,TM2,W,I,NS+1,NN-1,Wpos,Kxpos,mul,conv,nf,nk)
                ...
                if (zSold == zS):
                    if zSold > zRold:
                        ...
                        if zRold < zR:
                            ...
                            # (1) Medium above the source depth level: Reuse previous responses
                            RP1,RM1,TP1,TM1 = initials[2:6]
                            RP1,RM1,TP1,TM1 = PropScat(RP1,RM1,TP1,TM1,W,I,NRold+1,NR+1,Wpos,Kxpos,mul,conv,nf,nk)
                            
                            # (2) Medium between the source and receiver depth levels: Update previous responses
                            RP2,RM2,TP2,TM2 = PropScat(RP2,RM2,TP2,TM2,W,I,NR+1,NS+1,Wpos,Kxpos,mul,conv,nf,nk)
            ...
            if check == 1:
                ...
                # Compute medium responses
                RP1,RM1,TP1,TM1 = PropScat(RP1,RM1,TP1,TM1,W,I,Nstart,NR+1,Wpos,Kxpos,mul,conv,nf,nk)
                ...
                # Compute medium responses
                RP3,RM3,TP3,TM3 = PropScat(RP3,RM3,TP3,TM3,W,I,NS+1,NN-1,Wpos,Kxpos,mul,conv,nf,nk)   
        ...
        # The highest negative frequency and highest negative wavenumber components are real-valued
        GPP13[nf-1,:] = GPP13[nf-1,:].real
        GPM13[nf-1,:] = GPM13[nf-1,:].real
        GMP13[nf-1,:] = GMP13[nf-1,:].real
        GMM13[nf-1,:] = GMM13[nf-1,:].real
        
        GPP13[:,nk-1] = GPP13[:,nk-1].real
        GPM13[:,nk-1] = GPM13[:,nk-1].real
        GMP13[:,nk-1] = GMP13[:,nk-1].real
#        GMM13[:,nk-1] = GMM13[:,nk-1].real
        ...
        GPP13,GPM13,GMP13,GMM13 = self.Sort_kx_w(GPP13,GPM13,GMP13,GMM13)
    ...
    def Compose(self,GPP,GPM,GMP,GMM,cp=None,cs=None,ro=None,z=None,eps=None,initials=[]):
        """
        To be tested!
...
#        nt,nr,_ = np.shape(GPP)
        ...
        if check == 0:
            ...
            L1P = self.L1P_kx_w(cp,cs,ro,Kxpos,Wpos,nf,nk)
            L1M = self.L1M_kx_w(cp,cs,ro,Kxpos,Wpos,nf,nk)
            L2P = self.L2P_kx_w(cp,cs,ro,Kxpos,Wpos,nf,nk)
            L2M = self.L2M_kx_w(cp,cs,ro,Kxpos,Wpos,nf,nk)
            ...
            # N1M(+kx) = L2P(-kx).T = { -J L2P(+kx) J }.T = -{ J L2P(+kx) J }.T
        ...
        Gtf[nf-1,:,:] = Gtf[nf-1,:,:].real
        Gth[nf-1,:,:] = Gth[nf-1,:,:].real
        Gvf[nf-1,:,:] = Gvf[nf-1,:,:].real
        Gvh[nf-1,:,:] = Gvh[nf-1,:,:].real
        
        Gtf[:,nk-1,:] = Gtf[:,nk-1,:].real
        Gth[:,nk-1,:] = Gth[:,nk-1,:].real
        Gvf[:,nk-1,:] = Gvf[:,nk-1,:].real
        Gvh[:,nk-1,:] = Gvh[:,nk-1,:].real
        
        if (nt == self.nt) or (nr == self.nr):
            Gtf,Gth,Gvf,Gvh = self.Sort_kx_w(Gtf,Gth,Gvf,Gvh)
